...
################################################################################
covid <- readRDS("E:/Data/Training samples/st_log_reg_98_filtered/misinformation_class_15_98.RDS")
...
train |>
  ungroup() |>
  count(label) # misinfo = 15902, non misinfo = 202251

################################################################################
218153/(table(train$label)[1] * 2) # 6.859294               
218153/(table(train$label)[2] * 2) # 0.5393125             

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 6.859294, 0.5393125),
...
cm_lr$byClass["F1"] # 0.9834784                
cm_lr$byClass["Precision"] # 0.9994873               
cm_lr$byClass["Recall"] # 0.9679742              
...
lr_preds_all_filtered_label |> # 98
  ungroup() |>
  count(label) # misinfo = 429, nonmisinfo = 1673
...
covid_predicted |> # 98
  ungroup() |>
  count(label) # misinfo = 20232, nonmisinfo = 254562

saveRDS(covid_predicted, "E:/Data/Training samples/st_log_reg_98_filtered/misinformation_class_16_98.RDS")
