...
################################################################################
covid <- readRDS("~/INORK/Data/98_log_reg_misinfo_check.RDS")
...
train |>
  ungroup() |>
  count(label) # misinfo = 282, non misinfo = 876

################################################################################
1158/(table(train$label)[1] * 2) # 2.053191     
1158/(table(train$label)[2] * 2) # 0.6609589    

train <- train |>
  mutate(case_wts = ifelse(label == "misinfo", 2.053191 , 0.6609589 ),
...
cm_lr$byClass["F1"] # 0.6703911       
cm_lr$byClass["Precision"] # 0.8450704     
cm_lr$byClass["Recall"] # 0.5555556    
...
covid_predicted |> # 98
  ungroup() |>
  count(label) # misinfo = 364, nonmisinfo = 6112

saveRDS(covid_predicted, "~/INORK/NEW/Self_train/Log_reg/Results/misinformation_class_2_98.RDS")
